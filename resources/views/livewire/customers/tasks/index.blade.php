<div class="p-4 ">
    ...
    <ul class="flex flex-col gap-1 mb-6" wire:sortable="updateTaskOrder">
        @foreach($this->notDoneTasks as $task)
            <li wire:sortable.item="{{ $task->id }}" wire:key="task-{{ $task->id }}" class="">
                <livewire:customers.tasks.edit :task="$task" wire:key="task-edit-component-{{ $task->id }}" />
            </li>
        @endforeach
    </ul>
    ...
    <div x-data="{show: true}">
        <div class="uppercase font-bold text-slate-600 text-xs mb-2 flex items-center gap-2">
            ...
            <button title="{{ __('Hide/Show list') }}" class="cursor-pointer" @click="show = !show" type="button">
                <x-icon x-show="show" name="o-chevron-up"
                    class="w-4 h-4 -mt-px opacity-50 hover:opacity-100 hover:text-error" />
                <x-icon x-show="!show" name="o-chevron-down"
                    class="w-4 h-4 -mt-px opacity-50 hover:opacity-100 hover:text-error" />
            </button>
        </div>
                ...
                <li class="flex items-start gap-2 justify-between">
                    <div class="flex gap-2">
                        <input id="task-{{ $task->id }}" type="checkbox"
                            wire:click="toggleCheck({{ $task->id }}, 'pending')" value="1" @if($task->done_at) checked
                            @endif />
